...
std::tuple<int, int, int> Validator::validateDuration(string duration)
{
    ...
    if (firstDelimiterIndex == string::npos || lastDelimiterIndex == string::npos || firstDelimiterIndex == lastDelimiterIndex) {
        return std::make_tuple(-1, -1, -1); // Return an invalid duration if the ':' delimiter was not found or found only once
    }
    ...
    for (char i : hours) {
        if (!isdigit(i)) {
            return std::make_tuple(-1, -1, -1); // Return an invalid duration if the hours contain non-numeric characters
        }
    }
    ...
    for (char i : minutes) {
        if (!isdigit(i)) {
            return std::make_tuple(-1, -1, -1); // Return an invalid duration if the minutes contain non-numeric characters
        }
    }
    ...
    for (char i : seconds) {
        if (!isdigit(i)) {
            return std::make_tuple(-1, -1, -1); // Return an invalid duration if the seconds contain non-numeric characters
        }
    }
    int newSeconds = stoi(seconds);

    if (newMinutes < 0 || newMinutes > 59 || newSeconds < 0 || newSeconds > 59) {
        return std::make_tuple(-1, -1, -1); // Return an invalid duration if minutes or seconds are out of range
    }

    return std::make_tuple(newHours, newMinutes, newSeconds); // Return a tuple of the validated hours, minutes, and seconds
}
