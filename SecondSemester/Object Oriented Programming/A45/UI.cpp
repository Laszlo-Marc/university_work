...
void UI::DisplayTutorialsAdmin()
{
	for (int i = 0; i < this->service.GetSize(); i++) {
		...
		int seconds = 0;
		int hours = 0;
		std::tie(hours,minutes, seconds) = this->service.GetRepository()[i].getDuration();
		cout << i + 1 << ")  ""Title: " << this->service.GetRepository()[i].getTitle() << " Presenter: " <<
			this->service.GetRepository()[i].getPresenter() << " Duration:  " << hours <<":" << minutes << ":" << seconds
			<< " Likes: " << this->service.GetRepository()[i].getLikes() << "\nLink: "
			<< this->service.GetRepository()[i].getLink() << "\n" << "\n";
	}
}
...
void UI::DisplayTutorialsFromGivenPresenter()
{
	...
	if (this->validator.validatePresenter(presenter)) {
		for (int i = 0; i < this->service.GetSize(); i++) {
			if (this->service.GetRepository()[i].getPresenter()==presenter) {
				presenterCount++;
				int minutes = 0;
				int seconds = 0;
				int hours = 0;
				std::tie(hours, minutes, seconds) = this->service.GetRepository()[i].getDuration();
				cout << presenterCount << ")  ""Title: " << this->service.GetRepository()[i].getTitle() << " Presenter: " <<
					this->service.GetRepository()[i].getPresenter() << " Duration:  " << hours << ":" << minutes << ":" << seconds
					<< " Likes: " << this->service.GetRepository()[i].getLikes() << "\nLink: "
					<< this->service.GetRepository()[i].getLink() << "\n" << "\n";
				...
				cin >> choice;
				if (choice == 'w') {
					this->service.AddService(this->service.GetRepository()[i].getTitle(), presenter, this->service.GetRepository()[i].getLink(), this->service.GetRepository()[i].getLikes(), hours, minutes, seconds, 2);
					cout << "Tutorial added to watchlist!\n";
					CString cstr(this->service.GetRepository()[i].getLink().c_str());
...
void UI::DisplayTheWatchList()
{
	for (int i = 0; i < this->service.GetUserSize(); i++) {
		...
		int seconds = 0;
		int hours = 0;
		std::tie(hours, minutes, seconds) = this->service.GetUserRepository()[i].getDuration();
		cout << i + 1 << ")  ""Title: " << this->service.GetUserRepository()[i].getTitle() << " Presenter: " <<
			this->service.GetUserRepository()[i].getPresenter() << " Duration:  " << hours << ":" << minutes << ":" << seconds
			<< " Likes: " << this->service.GetUserRepository()[i].getLikes() << "\nLink: "
			<< this->service.GetUserRepository()[i].getLink() << "\n" << "\n";
	}
}
