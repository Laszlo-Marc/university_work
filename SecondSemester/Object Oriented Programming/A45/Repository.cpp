...
Repository::Repository(const DynamicArray<Tutorial>& array):tutorials{array}
{
...
DynamicArray<Tutorial>& Repository::getArray()
{
...
bool Repository::addRepo(string title, string presenter,int hours, int minutes,int seconds, string link, int likes)
{
	...
	if (position == -1) {
		...
		this->tutorials.addToArray(tutorial);
		return true;
	}
	return false;
}
...
int Repository::checkTutorialExistence(string title, string presenter, string link)
{
	for (int i = 0; i < this->tutorials.getSize(); i++) {
		if (this->tutorials[i].getTitle() == title && this->tutorials[i].getPresenter() == presenter && this->tutorials[i].getLink() == link)
			return i;
	}
	return -1;
}
...
bool Repository::deleteTutorialRepo( string title,string presenter,string link)
{
	...
	if (position != -1)
	{
		this->tutorials.deleteElem(position);
		return true;
	}
	else
		return false;
...
bool Repository::updateDurationRepo(string title, string presenter, string link,int hours, int minutes,int seconds)
{
	...
	if (position != -1) {
		...
		Tutorial tutorial(title, presenter, hours,minutes,seconds, link, likes);
		this->tutorials.update(tutorial, position);
		return true;
	}
	return false;
}
...
bool Repository::updateLikesRepo(string title, string presenter, string link,int likes)
{
	...
	if (position != -1) {
		...
		Tutorial tut(title, presenter, hours,minutes,seconds, link, likes);
		this->tutorials.update(tut, position);
		return true;
	}
	else
		return false;
